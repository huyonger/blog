...
const ALLOW_EXTS = [
    /** 图片文件 */
    /\.(gif|jpe?g|png|tiff|bmp|ico|svg)$/i,
    /** 多媒体文件 */
...
module.exports = class extends Base {
    ...
    async postAction() {
        ...
        // modify file.path:file.path + file.ext
        config.file = file;
        return this.serviceUpload(type, file, config);
    }
    ...
    async serviceImport(service, file) {
        ...
        return this.success(
            `共导入文章 ${ret.post} 篇，页面 ${ret.page} 页，分类 ${ret.category} 个，标签 ${ret.tag} 个`,
        );
    }
